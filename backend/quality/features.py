...
def extract_vehicle_features(
    vehicle_id: str,
    vehicle_history: Sequence[
        Union[VehicleObservation, Dict[str, Any]]
    ],
    ...
    """
    Constructs a vehicle-level feature dictionary representing all production
    conditions experienced by the vehicle strictly on or before as_of_timestamp.
    """

    valid_records = filter_by_timestamp(
        vehicle_history,
        as_of_timestamp,
    )
    ...
    if not valid_records:
        return {
            feat_name: 0.0
            for feat_name in QUALITY_FEATURE_NAMES
        }
    ...
    feats["station_exposure_duration"] = float(
        sum(durations)
    )

    feats["total_production_time"] = float(
        max(
            0.0,
            as_of_timestamp - min(timestamps),
        )
    )

    feats["stations_visited_count"] = float(
        len(unique_stations)
    )
    ...
    feats["cycle_time_mean"] = float(
        np.mean(cycle_times)
    )

    feats["cycle_time_max"] = float(
        np.max(cycle_times)
    )

    feats["cycle_time_deviation_mean"] = float(
        np.mean(cycle_deltas)
    )

    feats["cycle_time_deviation_max"] = float(
        np.max(cycle_deltas)
    )

    feats["cycle_time_deviation_cumulative"] = float(
        np.sum(
            np.maximum(
                0.0,
                cycle_deltas,
    ...
    feats["queue_exposure_mean"] = float(
        np.mean(queues)
    )

    feats["queue_exposure_max"] = float(
        np.max(queues)
    )

    feats["queue_exposure_cumulative"] = float(
        np.sum(queues)
    )

    feats["buffer_exposure_mean"] = float(
        np.mean(buffers)
    )

    feats["buffer_exposure_max"] = float(
        np.max(buffers)
    )
    ...
    feats["vibration_mean"] = float(
        np.mean(vibrations)
    )

    feats["vibration_max"] = float(
        np.max(vibrations)
    )

    feats["temperature_mean"] = float(
        np.mean(temperatures)
    )

    feats["temperature_max"] = float(
        np.max(temperatures)
    )

    feats["motor_current_mean"] = float(
        np.mean(currents)
    )

    feats["motor_current_max"] = float(
        np.max(currents)
    )

    feats["motor_current_variance_mean"] = float(
        np.mean(current_vars)
    )

    feats["motor_current_variance_max"] = float(
        np.max(current_vars)
    )
    ...
    if s3_records:
        ...
        feats["S3_torque_exposure"] = float(
            np.max(s3_torques)
        )
    ...
    feats["state_blocked_count"] = float(
        blocked_c
    )

    feats["state_starved_count"] = float(
        starved_c
    )

    feats["state_down_fault_count"] = float(
        down_fault_c
    )

    feats["state_warning_count"] = float(
        warning_c
    )

    feats["machine_state_abnormal_ratio"] = float(
        (
            blocked_c
            + starved_c
            + down_fault_c
    ...
    if phase4_adapter is not None:
        for st_id in unique_stations:
            summary = (
                phase4_adapter.get_station_summary(
                    station_id=st_id,
            ...
            p4_scores.append(
                summary.max_anomaly_score
            )

            p4_detected += (
                summary.anomaly_detected_count
            )

            if summary.severity_level == "HIGH":
                p4_high_sev += 1

    feats["phase4_anomaly_score_mean"] = (
        float(np.mean(p4_scores))
        if p4_scores
        else 0.0
    )

    feats["phase4_anomaly_score_max"] = (
        float(np.max(p4_scores))
        if p4_scores
        else 0.0
    )
    ...
    feats["phase4_high_severity_count"] = (
        float(p4_high_sev)
    )
    ...
    if phase5_adapter is not None:
        for st_id in unique_stations:
            p5_exp = (
                phase5_adapter.get_station_bottleneck_exposure(
                    station_id=st_id,
                    as_of_timestamp=as_of_timestamp,
                )
            )
            ...
            p5_risks.append(
                p5_exp["max_bottleneck_risk"]
            )

            p5_prop_risks.append(
                p5_exp["max_propagation_risk"]
            )

    feats[
        "phase5_bottleneck_risk_mean"
    ] = (
        float(np.mean(p5_risks))
        if p5_risks
        else 0.0
    )
    ...
    abnormal_events = (
        ...
        + (
            1
            if feats["queue_exposure_max"] >= 8
            else 0
        )
        + (
            1
            if feats["buffer_exposure_max"] >= 9.0
            else 0
        )
        + (
            1
            if feats["vibration_max"] >= 0.25
            else 0
        )
        + (
            1
    ...
    feats["number_of_abnormal_events"] = float(
        abnormal_events
    )
...
def feature_dict_to_array(
    ...
    """Converts feature dict to strict 1D numpy array."""

    names = (
        feature_names
        or QUALITY_FEATURE_NAMES
    )
...
class VehicleFeaturePreprocessor:
    ...
    def __init__(
        ...
        # Lazy import so sklearn is not loaded during API startup.
        from sklearn.preprocessing import StandardScaler

        self.feature_names: List[str] = (
            feature_names
            or list(QUALITY_FEATURE_NAMES)
        )
    ...
    def transform(
        ...
        if not self.is_fitted:
            raise RuntimeError(
                "Preprocessor must be fitted on training data before transform!"
            )
    ...
    def save(
        self,
        filepath_joblib: str,
        filepath_schema_json: Optional[str] = None,
    ) -> None:
