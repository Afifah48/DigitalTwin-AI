...
def calculate_calibration_error(
    ...
    y_p = np.clip(np.asarray(y_prob, dtype=float), 1e-6, 1.0 - 1e-6)

    prob_true, prob_pred = calibration_curve(y_t, y_p, n_bins=n_bins, strategy="uniform")

    # Expected Calibration Error (ECE)
    bin_edges = np.linspace(0.0, 1.0, n_bins + 1)
    bin_indices = np.digitize(y_p, bin_edges) - 1
    ...
    logloss = float(log_loss(y_t, y_p))
...
class QualityProbabilityCalibrator:
    ...
    def fit(self, base_estimator: Any, X_val: np.ndarray, y_val: np.ndarray) -> QualityProbabilityCalibrator:
        """
        Fits calibration model on validation predictions.
        """
        # Fit calibrator on raw probabilities or decision function
    ...
    def calibrate(self, raw_probs: np.ndarray) -> np.ndarray:
        ...
        probs_arr = np.asarray(raw_probs, dtype=float)
        if self.method == "isotonic":
            return np.clip(self.calibrator.predict(probs_arr), 0.0, 1.0)
        elif self.method == "sigmoid":
            return self.calibrator.predict_proba(probs_arr.reshape(-1, 1))[:, 1]
        return probs_arr
    ...
    @classmethod
    def load(cls, filepath: str) -> QualityProbabilityCalibrator:
