...
class OfflineEvaluator:
    ...
    def evaluate_dataset(
        ...
        # Import sklearn only when offline evaluation is actually run.
        ...
        if (
            ...
            or not os.path.exists(targets_path)
        ):
        ...
        # Index records into nested dict:
        # data_tree[ep_id][timestamp][station_id] = record
        data_tree: Dict[
            str,
        ...
        print(
            f"Running Phase 5 offline evaluation across "
            f"{len(episodes)} episodes..."
        )

        for ep_id in episodes:
            sc_type = ep_scenarios.get(
                ep_id,
                "UNKNOWN",
            )
            ...
            for t in sorted(
                ...
                analysis = (
                    self.pipeline.analyze_snapshot(
                        t,
                        station_tels,
                    )
                )
                ...
                for st_risk in analysis.station_ranking:
                    ...
                    st_id_str = (
                        st_risk.station_id.value
                    )

                    target_row = st_dict.get(
                        st_id_str
                    )
                    ...
                    if (
                        ...
                        and
                        st_risk.time_to_bottleneck_seconds
                        is not None
                    ):
                        gt_ttb = float(
                            target_row[
        ...
        results = {
            ...
            "mean_detection_lead_time_seconds": round(
                mean_lead_time_s,
                1,
            ),
            "mean_detection_lead_time_minutes": round(
                mean_lead_time_s / 60.0,
                2,
            ),
            "time_to_bottleneck_mae_seconds": round(
                mean_ttb_mae_s,
                1,
            ),
    ...
    def _print_evaluation_report(
        ...
        print(
            f"Total Observations Evaluated: "
            f"{results['total_observations']:,}"
        )

        print(
            f"Decision Threshold:           "
            f"{results['decision_threshold']:.2f}"
        )
        ...
        print(
            f"Mean Detection Lead Time:     "
            f"{results['mean_detection_lead_time_seconds']:.1f}s "
            f"({results['mean_detection_lead_time_minutes']:.1f} min)"
        )

        print(
            f"Time-to-Bottleneck MAE:       "
            f"{results['time_to_bottleneck_mae_seconds']:.1f}s"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Phase 5 Offline Bottleneck Evaluator"
    )
